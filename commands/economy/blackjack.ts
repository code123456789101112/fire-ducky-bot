...
export default {
    ...
    async execute(client: Client, message: Message, args: string[]) {
        ...
        const endGame = (result: { msg: string; win: boolean | null; }) => {
        ...
        const play = () => {
            ...
            const filter = (m: Message) => m.author.id === message.author.id && /^(h|s|e)/i.test(m.content);
            message.channel.awaitMessages(filter, { max: 1, time: 15000 }).then((collected: any) => {
