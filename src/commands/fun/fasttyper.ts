            ...
            message.channel.awaitMessages(m => m.content == phrase[random],
                { max: 1, time: 30000 }).then((collected: Collection<string, Discord.Message>) => {
                    if ((collected.first() as Message).content == phrase[random]) {
                        (collected.first() as Message).reply(`You win with a time of ${((collected.first() as Message).createdTimestamp - first.createdTimestamp) / 1000} seconds!!!!`);
                        return;
                    }
