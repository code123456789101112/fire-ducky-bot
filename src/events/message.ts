...
export default async (client: Client, message: Message): Promise<void> => {
	if (message.channel.type === "dm" && !message.author.bot) {
		...
		const collector = msg.createMessageComponentInteractionCollector(filter, { max: 1, time: 30000 });
		...
		collector.on("collect", async (interaction: MessageComponentInteraction) => {
			...
			const channel = await client.guilds.cache.get("824272560761733121")?.channels.create(`ticket-${interaction.user.username}`, {
				type: "text",
			...
			channel?.send({ content: "<@&824272561009328147>", embeds, components: [closeRow] });
